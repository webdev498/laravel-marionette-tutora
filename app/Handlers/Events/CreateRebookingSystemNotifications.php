...
class CreateRebookingSystemNotifications extends EventHandler
{
    ...
    public function handle(LessonBookingWasMade $event)
    {
        ...
        if ($remindAt) {
            $reminder = Reminder::make(Reminder::REBOOK, $remindAt);
    ...
    {
        $now      = Carbon::now();
        $minutes  = config('lessons.rebook_notification_period', 4320);
